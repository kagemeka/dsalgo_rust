...
impl<K: PartialOrd, V> AVLTree<K, V> {
    ...
    pub fn lower_bound(&self, key: &K) -> usize { Node::lower_bound(self.root.as_ref(), key) }

    pub fn upper_bound(&self, key: &K) -> usize { Node::upper_bound(self.root.as_ref(), key) }

    pub fn contains(&self, key: &K) -> bool { Node::find(self.root.as_ref(), key).is_some() }
}
