...
impl<K: PartialOrd, V> Node<K, V> {
    ...
    pub fn get_kth_node(root: &Box<Self>, k: usize) -> Option<&Box<Self>> {
        let current_index = Self::get_size(root.left.as_ref());
        if k == current_index {
            return Some(root);
        }
        if k < current_index {
            Self::get_kth_node(root.left.as_ref().unwrap(), k)
        } else if root.right.is_none() {
            return None;
        } else {
            Self::get_kth_node(root.right.as_ref().unwrap(), k - current_index - 1)
        }
    }
...
mod tests {
    ...
    fn test() {
        ...
        root = Node::remove(root, &2);
        println!("{:?}", root);
        assert_eq!(&Node::get_kth_node(root.as_ref().unwrap(), 1).unwrap().key, &3);
        assert_eq!(Node::get_kth_node(root.as_ref().unwrap(), 2), None);
