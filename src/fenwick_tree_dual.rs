...
impl<M: Monoid<S, T> + Commutative<S, T>, S: Clone, T> From<&[S]> for FenwickTreeDual<M, S, T> {
...
mod tests {
    ...
    fn test_as_abelian_group() {
        // use crate::abstract_traits::{BinaryOperation,
        ...
        // let mut fw = super::FenwickTree::<i32, i32,
        // Add>::from(arr.as_slice());
        ...
        // assert_eq!(fw.find_max_right_with_left(&is_ok, 0),
        // 6); let is_ok = |x: &i32| *x < 25;
        // assert_eq!(fw.find_max_right(&is_ok), 5);
        // assert_eq!(fw.find_max_right_with_left(&is_ok, 0),
        // 5); assert_eq!(fw.find_max_right_with_left(&
        // is_ok, 4), 6); assert_eq!(fw.
        // find_max_right_with_left(&is_ok, 5), 7);
        // assert_eq!(fw.find_max_right_with_left(&is_ok, 6),
        // 9); assert_eq!(fw.find_max_right_with_left(&
        // is_ok, 9), 10); assert_eq!(fw.
        // find_min_left_with_right(&is_ok, 10), 7);
        // assert_eq!(fw.find_min_left_with_right(&is_ok, 0),
        // 0); assert_eq!(fw.find_min_left_with_right(&
        // is_ok, 6), 2); assert_eq!(fw.
        // find_min_left_with_right(&is_ok, 5), 0); let
        // is_ok = |x: &i32| *x < 15; assert_eq!(fw.
        // find_max_right_with_left(&is_ok, 5), 5);
        // assert_eq!(fw.find_min_left_with_right(&is_ok, 6),
        // 6); assert_eq!(fw.find_min_left_with_right(&
        // is_ok, 10), 9); let is_ok = |x: &i32| *x <
        // 9; assert_eq!(fw.find_min_left_with_right(&
        // is_ok, 10), 10);
    }
}
